...
var EasySockError = errors.EasySockError;
...
var EasySock = function (config) {
  ...
  self.on('error', function (e) {
    ...
    console.error('default EasySock error handler:', e)
  })
...
EasySock.prototype.write = function (data, callback) {
  ...
  if (!Buffer.isBuffer(buf)) {
    return callback(new EasySockError("encode result is not Buffer"));
  }
  ...
  if (self.allowWriteTimeout) {
    self.timers.writes[seq] = setTimeout(function () {
      ...
      ctx.cb(new EasySockError("request timeout(" + self.timeout + "ms)"))
      self.deleteTask(seq)
    }, self.timeout);
  }
  // END 返回超时的逻辑
